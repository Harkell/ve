task :fetch_reading => :environment do 
require 'rubygems'	
require 'nokogiri'
require 'mechanize'
  agent = Mechanize.new

  page = agent.get("http://reading.escortdirectory-uk.com/")
  ...
  agent.page.links_with(text: /\d/).each do |link| # can select individual ages by (text: /1|2/)
    ...
    Escort.create(name: name, phone: phone, bio: bio, age: age, height: height, measurements: measurements, interests: interests,
     pictures: pictures, firstPriceRow: pricesOne, secondPriceRow: pricesTwo, thirdPriceRow: pricesThree, area_id: "69")

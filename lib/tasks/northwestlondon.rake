task :fetch_northwestlondon => :environment do 
require 'rubygems'	
require 'nokogiri'
require 'mechanize'
  agent = Mechanize.new

  page = agent.get("http://london.escortdirectory-uk.com/north-west-london")
  ...
  agent.page.links_with(text: /\d/).each do |link| # can select individual ages by (text: /1|2/)
    ...
    unless thread_page.parser.css('div.rates tr')[1] == nil
      pricesOne = thread_page.parser.css('div.rates tr')[1].to_s.encode('UTF-8')
    end
    ...
    Escort.create(name: name, phone: phone, bio: bio, age: age, height: height, measurements: measurements, interests: interests,
     pictures: pictures, firstPriceRow: pricesOne, secondPriceRow: pricesTwo, thirdPriceRow: pricesThree, area_id: "1")
  ...
  puts "------------------------------------"
  puts "Rake of North-west London complete."
